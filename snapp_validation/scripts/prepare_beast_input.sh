...
do
	...
	mkdir -p ../analysis/beast/ex3/root/${replicate_id}
	...
	cat tmp/part1.xml tmp/part2.xml > tmp/beast.xml
	rm tmp/part?.xml
	mv tmp/beast.xml ../analysis/beast/ex3/root/${replicate_id}
	rm -r tmp

	# Prepare XML files for analyses with younger constraints.
	mkdir -p ../analysis/beast/ex3/young/${replicate_id}
	...
	cat tmp/part1.xml tmp/part2.xml > tmp/beast.xml
	rm tmp/part?.xml
	mv tmp/beast.xml ../analysis/beast/ex3/young/${replicate_id}
	rm -r tmp
done