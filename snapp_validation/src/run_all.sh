...
# Prepare directories for SNAPP analyses (this is supposed to be done on abel, but can be done locally).
bash prepare_snapp_analyses.sh

# Prepare BEAST input for the concatenated analyses.
bash prepare_beast_input.sh

# Prepare directories for the concatenated BEAST analyses (this is supposed to be done on abel, but can be done locally).
bash prepare_beast_analyses.sh

# Run SNAPP analyses.
exit # This is done manually on abel.
...
# Get accuracy and precision for the estimates of the clock rate, theta, and the population size.
bash get_parameter_stats.sh
