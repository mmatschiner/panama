...
calibration_type = ARGV[3]
raise "ERROR: Unknown calibration type #{calibration_type}!" unless ["root","young"].include?(calibration_type)
...
if calibration_type == "root"
	node_calibrated_information << true
else
	node_calibrated_information << false
end
...
true_tree.branch.each do |ttb|
	unless ttb.extant
		...
		if calibration_type == "root"
			node_calibrated_information << false
		elsif calibration_type == "young"
			if ttb.id == selected_branch_id
				node_calibrated_information << true
			else
				node_calibrated_information << false
			end
...
true_ages.size.times do |x|
	output_string << "#{true_ages[x]}\t#{estimated_ages_mean[x]}\t#{estimated_ages_lowerHPD[x]}\t#{estimated_ages_upperHPD[x]}\t#{node_calibrated_information[x]}\n"
end
output_string << "\n"
puts output_string
